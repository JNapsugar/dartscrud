        ...
        {isPending ? (<div className="spinner-border"></div>) : (
            ...
            {dartses.map((darts, index) => (
                ...
                <div className="card h-100">
                    <p className="text-center">Dartsozó neve: {darts.name}</p>
                    <p className="text-center">Születési éve: {darts.birth_date}</p>
                    <p className="text-center">Megnyert világbajnokságai: {darts.world_ch_won}</p>
                    ...
                    <Link to={`/darts/${darts.id}`}>
                        <img src={darts.image_url} alt={darts.name} className="img-fluid" style={{ width: "200px" }} />
                    </Link>
